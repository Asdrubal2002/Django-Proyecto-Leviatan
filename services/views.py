    ...
    def post(self, request,slug, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                p, created = Work.objects.get_or_create(company=company,slug=slug,nombre=nombre,thumbnail=thumbnail,price=price)
                p.save()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                p, created = Empresa.objects.get_or_create(user=form.user,banner=banner,picture=picture,nombre=nombre,description=description,category=category,lugar=lugar,urlChat=urlChat,schedule=schedule,direction=direction,number=number,slug=slug)
                p.save()
                return redirect("services:company-detail", nombre=nombre)
